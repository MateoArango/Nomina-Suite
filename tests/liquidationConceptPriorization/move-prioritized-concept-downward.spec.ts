...
test.describe("Liquidation Concept Prioritization", () => {
  test("PLC-006: Move a prioritized concept downward", async ({ page }) => {
    ...
    test.skip(
      ...
      "PLC-006 requires at least two persisted prioritized concepts",
    );
    ...
    const selectedConceptId = initialOrderedIds[0];
    const followingConceptId = initialOrderedIds[1];
    expect(selectedConceptId).toBeDefined();
    expect(followingConceptId).toBeDefined();
    ...
    try {
      ...
      expect(reorderedIds[0]).toBe(followingConceptId);
      expect(reorderedIds[1]).toBe(selectedConceptId);
